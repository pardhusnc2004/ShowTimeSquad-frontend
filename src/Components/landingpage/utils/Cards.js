          ...
          <Col key={card.name}>
            <Card style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              <div style={{ flex: '50', overflow: 'hidden' }}>
                <Card.Img variant="top" src={card.image} alt={`Card ${card.id}`} style={{ objectFit: 'cover', height: '100%', width: '100%' }} />
              </div>
              <Card.Body>
                <Card.Text>{card.name}</Card.Text>
                <Button variant="primary" id={card._id} onClick={getMovieDetails}>Book Now</Button>
              </Card.Body>
            </Card>
          </Col>
        ))}
