...
const MovieDetails = () => {
  ...
  useEffect(() => {
    ...
    backgroundImage.onload = function () {
      ...
      const [r, g, b] = ctx.getImageData(0, 0, backgroundImage.width, backgroundImage.height).data;
      const luminance = calculateLuminance(r / 255, g / 255, b / 255);
      setTextColor(luminance > 0.5 ? 'text-dark' : 'text-light');
    };
    Axios.get('https://showtimesquad-backend.onrender.com/movies/get-details/'+id).then((res)=>{
      if(res.status===200){
        setData(res.data);
      }
      else{
  ...
  return (
    ...
    >
      <Container className="mt-4">
        <Row>
          <Col md={3}>
            <Card className="movie-poster-card border-0" style={{ background: 'rgba(255,255,255,0)' }}>
              {/* IMDb Rating */}
              <div
              ...
              >
                <FontAwesomeIcon icon={faImdb} className="fa" style={{fontSize: "36px"}} />
              ...
              <Card.Img
                ...
                className="img-fluid"
              />
              <Card.Body style={{ background: 'rgba(255,255,255,0)', borderRadius: '8px'}}>
                <Card.Title className={`text-center ${textColor}`}>{data.name}</Card.Title>
          ...
          <Col md={9}>
            {/* Movie Details */}
            <Card className="movie-details-card border-0" style={{ background: 'rgba(255,255,255,0)', borderRadius: '8px' }}>
              <Card.Body>
